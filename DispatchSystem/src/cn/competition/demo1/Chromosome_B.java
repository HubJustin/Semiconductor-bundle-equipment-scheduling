package cn.competition.demo1;

import java.util.ArrayList;
import java.util.List;

public class Chromosome_B {
    private boolean[] gene;//基因序列
    private int score;//对应的函数得分

    public int getScore() {
        return score;
    }

    public void setScore(int score) {
        this.score = score;
    }

    public void setGene(boolean[] gene) {
        this.gene = gene;
    }

    //随机生成基因序列
    public Chromosome_B(int size) {
        if (size <= 0) {
            return;
        }
        initGeneSize(size);
        for (int i = 0; i < size; i++) {
            gene[i] = Math.random() >= 0.5;
        }
    }

    //生成一个新基因
    public Chromosome_B() {

    }

    //克隆基因
    public static Chromosome_B clone(final Chromosome_B c) {
        if (c == null || c.gene == null) {
            return null;
        }
        Chromosome_B copy = new Chromosome_B();
        copy.initGeneSize(c.gene.length);
        for (int i = 0; i < c.gene.length; i++) {
            copy.gene[i] = c.gene[i];
        }
        return copy;
    }

    //初始化基因长度
    private void initGeneSize(int size) {
        if (size <= 0) {
            return;
        }
        gene = new boolean[size];
    }

    //遗传产生下一代
    public static List<Chromosome_B> genetic(Chromosome_B p1, Chromosome_B p2) {
        if (p1 == null || p2 == null) { //染色体有一个为空，不产生下一代
            return null;
        }
        if (p1.gene == null || p2.gene == null) { //染色体有一个没有基因序列，不产生下一代
            return null;
        }
        if (p1.gene.length != p2.gene.length) { //染色体基因序列长度不同，不产生下一代
            return null;
        }
        Chromosome_B c1 = clone(p1);
        Chromosome_B c2 = clone(p2);
        //随机产生交叉互换位置
        int size = c1.gene.length;
        int a = ((int) (Math.random() * size)) % size;
        int b = ((int) (Math.random() * size)) % size;
        int min = a > b ? b : a;
        int max = a > b ? a : b;
        //对位置上的基因进行交叉互换
        for (int i = min; i <= max; i++) {
            boolean t = c1.gene[i];
            c1.gene[i] = c2.gene[i];
            c2.gene[i] = t;
        }
        List<Chromosome_B> list = new ArrayList<Chromosome_B>();
        list.add(c1);
        list.add(c2);
        return list;
    }

    //基因num个位置发生变异
    public void mutation(int num) {
        //允许变异
        int size = gene.length;
        for (int i = 0; i < num; i++) {
            //寻找变异位置
            int at = ((int) (Math.random() * size)) % size;
            //变异后的值
            boolean bool = !gene[at];
            gene[at] = bool;
        }
    }

    //将基因转化为对应的序列
    public List<Ordering_Bmachine> getOrderingBmachine(int translateMode) {
        String finalPosition0,finalPosition1,finalPosition2,finalPosition3;
        List<Ordering_Bmachine> orderingBmachines = new ArrayList<>();
        if (gene == null) {
            return null;
        }
        for (int i = 0; i < gene.length/4; i++) {
            if (gene[4*i+0])
                finalPosition0="LLA";
            else
                finalPosition0="LLB";

            if (gene[4*i+1])
                finalPosition1="PM7";
            else
                finalPosition1="PM8";

            if (translateMode==0){
                if (i<25){
                    if (gene[4*i+2])
                        finalPosition2="PM4";
                    else
                        finalPosition2="PM5";
                }
                else if (i<50){
                    if (gene[4*i+2])
                        finalPosition2="PM9";
                    else
                        finalPosition2="PM10";
                }
                else
                    finalPosition2=null;
            }else if (translateMode==1){
                if (gene[4*i+2])
                    finalPosition2="PM4";
                else
                    finalPosition2="PM5";
            }else if (translateMode==2){
                if (gene[4*i+2])
                    finalPosition2="PM9";
                else
                    finalPosition2="PM10";
            }else
                finalPosition2=null;

            if (gene[4*i+3])
                finalPosition3="LLA";
            else
                finalPosition3="LLB";

            orderingBmachines.add(new Ordering_Bmachine(finalPosition0,finalPosition1,finalPosition2,finalPosition3));
        }
        return orderingBmachines;
    }

}

